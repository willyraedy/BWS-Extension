...
chrome.tabs.query({ 'active': true, 'lastFocusedWindow': true }, function (tabs) {
  ...
  chrome.runtime.sendMessage({type: 'request-company-details', domain}, function(response){
    ...
    if (currentCompany) {
      setGradeContent(currentCompany);
      ...
      if (companyHasBadGrade(currentCompany)) {
        ...
        if (paused) {
          ...
          if (whitelist.includes(currentCompany.domain)) {
            createRemoveFromWhitelistButton(domain, tabId)
          } else {
            createAddToWhitelistButton(domain, currentCompany);
          }
        }
      } else {
...
function companyHasBadGrade(currentCompany) {
  return currentCompany.grade.search(/D|F/) !== -1
}
...
function createPauseButton() {
  $('#pause-btn').text('Pause Better World Shopping Guide')
...
function createUnpauseButton() {
  $('#pause-btn').text('Unpause Better World Shopping Guide')
  $('#pause-btn').on('click', function () {
...
function createAddToWhitelistButton(domain, currentCompany) {
  $('#whitelist-btn').text(`Stop modals from appearing on ${domain}.com`);
  $('#whitelist-btn').on('click', function () {
...
function createRemoveFromWhitelistButton(domain, tabId) {
  $('#whitelist-btn').text(`Reactivate modals on ${domain}.com`);
  $('#whitelist-btn').on('click', function () {
    chrome.storage.local.get('whitelist', function (result) {
...
function setGradeContent(currentCompany) {
